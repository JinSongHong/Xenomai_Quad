...
void Controlwindow::on_Set_clicked() { // click event -> all gain are set at once
    ...
    if(ui->Leg_ctrl_mode->isChecked() && !ui->Trunk_ctrl_mode->isChecked()){Trunk_Leg_CtrlMode = 0;}
    else if(!ui->Leg_ctrl_mode->isChecked() && ui->Trunk_ctrl_mode->isChecked()){Trunk_Leg_CtrlMode = 1;}
    else {
    QMessageBox msgBox;
        msgBox.setText("Check the control mode!");
        msgBox.setWindowTitle("Error");
    ...
    /******* RWDOB ********/
    RW_DOB_cutoff[0] = ui->FL_DOB_cutoff->value();
    RW_DOB_cutoff[1] = ui->FR_DOB_cutoff->value();
    RW_DOB_cutoff[2] = ui->RL_DOB_cutoff->value();
    RW_DOB_cutoff[3] = ui->RR_DOB_cutoff->value();
    
    /******* RWFOB ********/
    RW_FOB_cutoff[0] = ui->FL_FOB_cutoff->value();
    RW_FOB_cutoff[1] = ui->FR_FOB_cutoff->value();
    RW_FOB_cutoff[2] = ui->RL_FOB_cutoff->value();
    RW_FOB_cutoff[3] = ui->RR_FOB_cutoff->value();
    
    //// Orientaiton DOB ////
    Ori_DOB_cutoff[0] = ui->FL_Ori_DOB_cutoff->value();
    Ori_DOB_cutoff[1] = ui->FR_Ori_DOB_cutoff->value();
    Ori_DOB_cutoff[2] = ui->RL_Ori_DOB_cutoff->value();
    Ori_DOB_cutoff[3] = ui->RR_Ori_DOB_cutoff->value();
}
...
void Controlwindow::Mutex_exchange()
{
    ...
    if (!ret) {
      ...
      for (int i = 0; i < 4; i++) {
        ...
        _M_RW_r_velPgain[i] = RW_r_velPgain[i];
        _M_RW_r_velDgain[i] = RW_r_velDgain[i];
        _M_RW_r_velIgain[i] = RW_r_velIgain[i];
        _M_RW_r_velD_cutoff[i] = RW_r_velD_cutoff[i];
        _M_RW_th_velPgain[i] = RW_th_velPgain[i];
        _M_RW_th_velIgain[i] = RW_th_velIgain[i];
        _M_RW_th_velDgain[i] = RW_th_velDgain[i];
        _M_RW_th_velD_cutoff[i] = RW_th_velD_cutoff[i];
        
        _M_RW_DOB_cutoff[i] = RW_DOB_cutoff[i];
        _M_RW_FOB_cutoff[i] = RW_FOB_cutoff[i];
      }
      _M_Ori_DOB_cutoff = Ori_DOB_cutoff;
      ...
      _M_ctrl_mode = CtrlMode;
      _M_Trunk_Leg_CtrlMode = Trunk_Leg_CtrlMode;
...
void Controlwindow::on_IMU_on_clicked()
{
  if(ui->IMU_on->isChecked())
    IMU_on = true;
}
